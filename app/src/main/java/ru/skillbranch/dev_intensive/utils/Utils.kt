...
object Utils {
    fun parseFullName(fullName: String?): Pair<String?, String?>{
        ...
        return (parts?.getOrNull(0)?.trim() ?: null) to (parts?.getOrNull(1)?.trim()?: null)
    }

    fun toInitials(firstName:String?, lastName:String?): String?{
        if(firstName.isNullOrBlank() && lastName.isNullOrBlank()) return null

        var result = (firstName?.trim()?.substring(0,1)?.toUpperCase() ?: "") + (lastName?.trim()?.substring(0,1)?.toUpperCase() ?: "")
        return result
    }
